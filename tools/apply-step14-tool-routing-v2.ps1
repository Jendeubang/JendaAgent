...
$replacements = @(
    [pscustomobject]@{
        Path = Join-Path $root "genie-backend/src/main/java/com/jd/genie/model/agent/AgentEventType.java"
        Old = @'
    TASK("task"),
...
'@
    ...
    [pscustomobject]@{
        Path = Join-Path $root "showcase/app/live-multimodal/page.tsx"
        Old = @'
      task: "子任务执行",
      tool_result: "工具结果",
'@
...
# Validate every target before writing so this never leaves a partial Step 14 change.
