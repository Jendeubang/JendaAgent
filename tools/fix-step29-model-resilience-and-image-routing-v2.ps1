...
$1        if (request.image_urls() == null || request.image_urls().isEmpty()) {
            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "image_edit requires at least one image_url");
        }
$2
'@.TrimEnd()
Replace-OneRegex "src/main/java/com/jd/genie/controller/QwenImageEditToolGatewayController.java" $controllerPattern $controllerReplacement

Write-Host "Step 29 v2 completed. Rebuild and restart the backend."
