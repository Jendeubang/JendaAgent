...
$path = Join-Path $root "genie-backend\src\main\java\com\jd\genie\service\agent\AgentAssetMetadataStore.java"
...
/** Persists COS asset ownership independently of temporary signed URLs. */
@Repository
public class AgentAssetMetadataStore {
    ...
    public void recordGeneratedForSession(String sessionId, String runId, String assetId, String title, String imageUrl) {
        ...
        upsert(new AgentAssetMetadata(assetId, owners.get(0), sessionId, runId, title, "image/*", 0L, null, imageUrl, "generated", Instant.now()));
    }

    public Optional<AgentAssetMetadata> findOwned(String ownerUserId, String assetId) {
        List<AgentAssetMetadata> assets = jdbcTemplate.query("SELECT asset_id, owner_user_id, session_id, run_id, file_name, media_type, size_bytes, object_key, image_url, source, created_at FROM agent_asset WHERE asset_id = ? AND owner_user_id = ?", (resultSet, rowNum) -> new AgentAssetMetadata(resultSet.getString("asset_id"), resultSet.getString("owner_user_id"), resultSet.getString("session_id"), resultSet.getString("run_id"), resultSet.getString("file_name"), resultSet.getString("media_type"), resultSet.getLong("size_bytes"), resultSet.getString("object_key"), resultSet.getString("image_url"), resultSet.getString("source"), resultSet.getTimestamp("created_at").toInstant()), assetId, ownerUserId);
    ...
    private void upsert(AgentAssetMetadata asset) {
        int updated = jdbcTemplate.update(connection -> {
            ...
            setValues(statement, asset, false);
            return statement;
        });
        if (updated == 0) {
    ...
    private void setValues(PreparedStatement statement, AgentAssetMetadata asset, boolean unused) throws java.sql.SQLException {
        statement.setString(1, asset.ownerUserId());
        statement.setString(2, asset.sessionId());
        statement.setString(3, asset.runId());
        statement.setString(4, asset.fileName());
        statement.setString(5, asset.mediaType());
        statement.setLong(6, asset.size());
        statement.setString(7, asset.objectKey());
        statement.setString(8, asset.imageUrl());
        statement.setString(9, asset.source());
        statement.setTimestamp(10, Timestamp.from(asset.createdAt()));
        statement.setString(11, asset.assetId());
    }
}
'@
[System.IO.File]::WriteAllText($path, $source.TrimStart([char]13, [char]10) + "`n", $utf8)
Write-Host "Replaced asset metadata writes with MySQL 5.1-compatible setTimestamp calls. Rebuild the backend."
