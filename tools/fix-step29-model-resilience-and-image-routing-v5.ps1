...
$clientCatchReplacement = '        } catch (Exception error) {' + [Environment]::NewLine + '            // Planning and summary are advisory. A provider outage must not prevent tool execution or history persistence.' + [Environment]::NewLine + '            return ModelCompletion.skipped("Model temporarily unavailable; using rule routing: " + (error.getMessage() == null ? error.getClass().getSimpleName() : error.getMessage()));' + [Environment]::NewLine + '        }'
...
$missingValidation = '        if (request.image_urls() == null || request.image_urls().isEmpty()) {' + [Environment]::NewLine + '            throw new ResponseStatusException(HttpStatus.BAD_REQUEST, "image_edit requires at least one image_url");' + [Environment]::NewLine + '        }'
...
Write-Host "Step 29 v5 completed. Rebuild and restart the backend."
