...
# This continuation is safe after apply-step30-asset-center.ps1 stopped at COS injection.
# It validates every target before writing either source file.
...
$imageUrlPattern = '(?ms)^    public String imageUrl\(StoredAgentImage image\) \{\R        return baseUrl \+ "/" \+ encodeObjectKey\(image\.storedFileName\(\)\);\R    \}'
