...
function Replace-RegexOnce([string]$Pattern, [string]$Replacement) {
  $matches = [regex]::Matches($script:content, $Pattern)
  if ($matches.Count -ne 1) { throw "Expected exactly one matching pattern in $path, found $($matches.Count). No file was changed." }
  $script:content = [regex]::Replace($script:content, $Pattern, [System.Text.RegularExpressions.MatchEvaluator]{ param($match) $Replacement }, 1)
}

Replace-RegexOnce '(?m)^import org\.springframework\.stereotype\.Service;\r?$' "import org.springframework.stereotype.Service;`r`nimport lombok.extern.slf4j.Slf4j;"
Replace-RegexOnce '(?m)^import java\.time\.Instant;\r?$' "import java.time.Duration;`r`nimport java.time.Instant;"
Replace-RegexOnce '(?m)^@Service\r?$' "@Service`r`n@Slf4j"
Replace-RegexOnce '(?m)^    private final OkHttpClient client = new OkHttpClient\(\);\r?$' @'
...
$putPattern = '(?s)    private void putObject\(String objectKey, String contentType, byte\[\] bytes\) \{.*?(?=    private String authorization\()'
$putReplacement = @'
...
'@
Replace-RegexOnce $putPattern $putReplacement
