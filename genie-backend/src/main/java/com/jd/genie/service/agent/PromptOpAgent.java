 ...
 * It retrieves applicable prompt rules before asking the configured Qwen chat model for a production-ready rewrite.
 */
@Slf4j
@Component
...
public class PromptOpAgent {
    ...
    public PromptOptimization optimize(String originalPrompt, List<AgentToolType> tools, List<String> imageUrls) {
        if (!properties.isEnabled()) {
            return PromptOptimization.skipped(originalPrompt, "PromptOpAgent is disabled");
        }
        List<AgentToolType> imageTools = tools.stream()
                .filter(tool -> tool == AgentToolType.IMAGE_GENERATE || tool == AgentToolType.IMAGE_EDIT)
                .toList();
        if (imageTools.isEmpty()) {
            return PromptOptimization.skipped(originalPrompt, "No image tool is scheduled");
        }
        List<PromptKnowledgeSnippet> snippets = knowledgeRetriever.retrieve(originalPrompt, imageTools, properties.getMaxKnowledgeItems());
        ...
        try {
            ...
            if (!completion.invoked()) {
                return new PromptOptimization(false, originalPrompt, originalPrompt, ruleIds, completion.provider(), completion.content());
            }
            String optimized = normalize(completion.content(), originalPrompt);
            return new PromptOptimization(true, originalPrompt, optimized, ruleIds, completion.provider(), "Prompt optimized with retrieved guidelines");
        } catch (Exception error) {
            log.warn("PromptOpAgent fell back to the original prompt: {}", error.getMessage());
            return new PromptOptimization(false, originalPrompt, originalPrompt, ruleIds, "fallback", "Prompt optimization failed; original prompt retained");
        }
    }
    ...
    private String systemPrompt(List<AgentToolType> tools) {
        ...
        return "You are PromptOpAgent. Rewrite the user's request as one precise production image prompt. "
                ...
                + (editing
                ? "This is an image-edit task. Preserve unmentioned regions and identity. If multiple images exist, respect their order and any mask instruction."
    ...
    private String userPrompt(String originalPrompt, List<AgentToolType> tools, List<PromptKnowledgeSnippet> snippets) {
        String guidelines = snippets.isEmpty()
                ? "No matching guidelines were retrieved."
                : snippets.stream().map(snippet -> "[" + snippet.id() + "] " + snippet.guidance())
