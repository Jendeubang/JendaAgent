...
public class PersistentAgentWorkspaceCatalog {
    ...
    public Optional<AgentWorkspaceSnapshot> workspace(String ownerUserId, String sessionId) {
        ...
        jdbcTemplate.query("SELECT asset_id, run_id, file_name, image_url, object_key, created_at FROM agent_asset WHERE session_id = ? AND owner_user_id = ? AND source <> 'upload' ORDER BY created_at DESC", (RowCallbackHandler) resultSet -> assets.add(new AgentWorkspaceAsset(resultSet.getString("asset_id"), resultSet.getString("run_id"), resultSet.getString("file_name"), refreshUrl(resultSet.getString("object_key"), resultSet.getString("image_url")), "generated", instant(resultSet.getTimestamp("created_at")))), sessionId, ownerUserId);
