...
test("login stores the user session and enters the agent workspace", async ({ page }) => {
  await page.route("**/api/v1/auth/captcha", async (route) => route.fulfill({ contentType: "application/json", body: JSON.stringify(captcha) }));
